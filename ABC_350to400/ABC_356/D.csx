...
class Kyopuro {
	...
	public void Solve() {
		...
		for (int i = 0; m >> i > 0; ++i) {
			...
			if ((tei >> 1) > n) break;
			// とりあえずブロックで
			long ue = (n + 1l) / tei * tei / 2l;
			ans = (ans + ue) % m998;
			// modを引く
			// long nokori = (n + nokori_p) % tei;
			// nokori = Max(nokori - (tei >> 1) + 1, 0l);
			// ans = (ans + nokori) % m998;
			long nokori = (n + 1) % tei;
			nokori = Max(nokori - (tei >> 1), 0l);
			ans = (ans + nokori) % m998;
			// writeline($"ans:{ans} tei:{tei} ue:{ue} sita:{nokori}");
		}
