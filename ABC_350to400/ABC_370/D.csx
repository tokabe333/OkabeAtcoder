...
class Kyopuro {
	...
	public void Solve() {
		var (h, w, q) = readintt3();
		var rows = new AVLTree<int>[h];
		for (int i = 0; i < h; ++i) rows[i] = new AVLTree<int>();
		var cols = new AVLTree<int>[w];
		for (int i = 0; i < w; ++i) cols[i] = new AVLTree<int>();
		...
		for (int _ = 0; _ < q; ++_) {
			var rrcc = read().Split(' ').Select(x => int.Parse(x) - 1).ToArray();
			int r = rrcc[0];
			int c = rrcc[1];

			// return;

			if (rows[r].Contains(c) == true) {
				rows[r].Remove(c);
				cols[c].Remove(r);
				ans -= 1;
			} else {
				// u
				bool found = true;
				int ur = cols[c].FindLessThanMax(r, out found);
				if (found == true) {
					cols[c].Remove(ur);
					rows[ur].Remove(c);
					ans -= 1;
				...
				// d
				int dr = cols[c].UpperBound(r, out found);
				if (found == true) {
					cols[c].Remove(dr);
					rows[dr].Remove(c);
					ans -= 1;
				...
				// l
				int lc = rows[r].FindLessThanMax(c, out found);
				if (found == true) {
					rows[r].Remove(lc);
					cols[lc].Remove(r);
					ans -= 1;
				...
				// r
				int rc = rows[r].UpperBound(c, out found);
				if (found == true) {
					rows[r].Remove(rc);
					cols[rc].Remove(r);
					ans -= 1;
