...
class Kyopuro {
	...
	public void Solve() {
		...
		var mincosts = new Dictionary<long, long>();
		long min = iinf;
		for (long i = n - 1; i >= 0; --i) {
			min = Min(min, cards[i][1]);
			mincosts[cards[i][0]] = min;
		}

		var keys = mincosts.Keys.ToArray();
		Array.Sort(keys);
		...
		for (long i = 0; i < n; ++i) {
			...
			if (cards[i][0] == keys[index]) index += 1;
			if (index == keys.Length) {
				ans.Add(cards[i][2]);
				continue;
			}

			// writeline($"cards[{i}][1]:{cards[i][1]} keys[{index}]:{keys[index]}");
			if (cards[i][1] <= keys[index]) ans.Add(cards[i][2]);
		}
