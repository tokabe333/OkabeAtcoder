...
class Kyopuro {
	...
	public void Solve() {
		...
		var avl = new AVLTree<long>(arr);

		int check(long m, long b) {
			long l = b - m;
			long r = b + m;
			bool ret1;
			var it1 = avl.LowerBoundIterator(l, out ret1);
			if (ret1 == false) return 0;
			if (l < it1.value.key) {
				it1 = it1.Prev();
				if (it1.value == null) return 0;
			}

			bool ret2;
			var it2 = avl.FindLessThanMaxIterator(r + 1, out ret2);
			if (ret2 == false) return 0;
			if (it2.value.key < r) {
				it2 = it2.Next();
				if (it2.value == null) return 0;
			}

			int li = dict[it1.value.key];
			int ri = dict[it2.value.key];
