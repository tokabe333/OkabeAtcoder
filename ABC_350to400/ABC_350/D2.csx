...
class Kyopuro {
	...
	public void Solve() {
		...
		void dfs(int start, int current, int depth) {
			if (depth == 2) {
				if (setgraph[start].Contains(current)) return;
				ans += 1;
				graph[start].Add(current);
				graph[current].Add(start);
				setgraph[start].Add(current);
				setgraph[current].Add(start);
				// writeline($"start:{start + 1} current:{current + 1} ");
				return;
			}
			for (int i = 0; i < graph[current].Count; ++i) {
				if (graph[current][i] == start) continue;
				dfs(start, graph[current][i], depth + 1);
			}
		}
