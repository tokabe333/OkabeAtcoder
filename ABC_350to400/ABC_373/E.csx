...
class Kyopuro {
	...
	public void Solve() {
		...
		for (int i = 0; i < n; ++i) {
			long sumup2 = set.Contains(arr[i]) ? sumup - arr[i] + sorted[m] : sumup;

			if ((arr[i] + nokori) * m < sumup2) {
				write(-1 + " ");
				continue;
			}
			if (arr[i] * m >= sumup2 + nokori) {
				write(0 + " ");
				continue;
			}
			...
			while (Abs(l - r) > 1) {
				long get = (l + r) / 2;
				// 確定？
				bool check = (arr[i] + get) * m >= (sumup2 + (nokori - get));
			...
			// 確定してなければ +1
			if (arr[i] + l < (sumup2 + (nokori - l)) / m) l += 1;
