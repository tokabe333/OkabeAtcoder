...
class Kyopuro {
	...
	public void Solve() {
		...
		// for (int i = 0; i < 200; ++i) {
		// 	if (mods[i].Count <= 1) continue;
		// 	writeline($"{i} {IntToString2bit(mods[i][0])} {IntToString2bit(mods[i][1])}");
		// }
		...
		for (int i = 0; i < 200; ++i) {
			...
			for (int j = 0; j < n; ++j) {
				if (((1 << j) & mods[i][0]) != 0) ans1.Add(j + 1);
				if (((1 << j) & mods[i][1]) != 0) ans2.Add(j + 1);
			}
			writeline("Yes");
			writeline(ans1.Count + " " + string.Join(" ", ans1));
			writeline(ans2.Count + " " + string.Join(" ", ans2));
			return;
		}
		writeline("No");
