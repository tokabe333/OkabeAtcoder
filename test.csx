...
class Kyopuro {
	...
	void dfs(int start, int node, HashSet<int> set) {
		set.Add(node);
		...
		// write("node:" + node + " setcount:" + set.Count + " ");
		// foreach (var s in set) write(s + " ");
		// writeline();

		foreach (var next in graph[node]) {
			if (set.Contains(next) == false) dfs(start, next, set);
		}
		set.Remove(node);
	}
