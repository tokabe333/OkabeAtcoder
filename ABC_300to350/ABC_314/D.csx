...
public class Kyopuro {
	public static void Main() {
		...
		var trr = new int[q];
		var xrr = new int[q];
		var crr = makearr<char>(q, '_');

		for (int i = 0; i < q; ++i) {
			var line = readstrings();
			trr[i] = int.Parse(line[0]);
			xrr[i] = int.Parse(line[1]) - 1;
			if (trr[i] == 1) crr[i] = line[2][0];
		}

		int kakikae = 0;
		int index = 0;
		for (int i = q - 1; i >= 0; --i) {
			if (trr[i] == 2 && kakikae == 0) {
				kakikae = -1;
				index = i;
			} else if (trr[i] == 3 && kakikae == 0) {
				kakikae = 1;
				index = i;
			}
		}
		...
		// printlist(crr);
		// println("");
		// println(kakikae + " " + index);
		for (int i = 0; i < n; ++i) {
			s[i] = kakikae == 1 ? char.ToUpper(s[i]) : char.ToLower(s[i]);
		}

		for (int i = 0; i < q; ++i) {
			if (crr[i] == '_') continue;
			if (i <= index) s[xrr[i]] = kakikae == 1 ? char.ToUpper(crr[i]) : char.ToLower(crr[i]);
			else s[xrr[i]] = crr[i];
		}
