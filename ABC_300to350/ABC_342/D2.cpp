...
int main() {
    ...
    vll heihou;
    ll  hei = 0;
    while (hei * hei <= 4 * pow(10, 10)) {
        // heihou.insert(hei * hei);
        heihou.emplace_back(hei * hei);
        hei += 1;
    }

    cout << "hoge" << endl;
    vector<unordered_set<ll>> heihous(heihou.size());
    rep(i, heihou.size()) {
        heihous[i] = divisor(heihou[i]);
    }
