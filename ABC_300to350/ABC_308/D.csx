...
class Kyopuro {
	...
	public void Solve() {
		var (h, w) = readintt2();
		var snuke = new string[h];
		for (int i = 0; i < h; ++i) snuke[i] = read();
		...
		for (int i = 0; i < h; ++i) {
			for (int j = 0; j < w; ++j) {
				masu[i][j] = dict.ContainsKey(snuke[i][j]) ? dict[snuke[i][j]] : 9;
			}
		}
		...
		while (que.Count > 0) {
			...
			if (masu[y][x] > 4) continue;
			int next = (masu[y][x] + 1) % 5;
			masu[y][x] = 9;

			// u,d,l,r
			if (0 < y && masu[y - 1][x] == next) que.Enqueue(y * w + x - w);
			if (y < h - 1 && masu[y + 1][x] == next) que.Enqueue(y * w + x + w);
			if (0 < x && masu[y][x - 1] == next) que.Enqueue(y * w + x - 1);
			if (x < w - 1 && masu[y][x + 1] == next) que.Enqueue(y * w + x + 1);
		}
