...
int main() {
    ...
    rep(start, n) {
        ...
        for (int diff = 0; diff < n / 2; ++diff) {
            int index1 = (start + diff) % n;
            int index2 = (start + (n - diff - 1)) % n;

            // cout << "diff:" << diff << " ind1:" << index1 << " ind2:" << index2 << " ind1c:" << str[index1] << " ind2c:" << str[index2] << endl;

            if (str[index1] != str[index2]) repr += b;
        }
        ans = min(ans, idou + repr);
    }
