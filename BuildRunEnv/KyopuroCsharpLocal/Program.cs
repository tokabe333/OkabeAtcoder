...
<<<<<<< HEAD
		...
		/// k は現在のノード番号
		/// [a, b) はkに対応する半開区間
		public long Query(long l, long r, long k, long a, long b) {
			// 現在の対応ノード区間が求めたい区間に含まれないとき
			// → 単位元を返す
			if (r <= a || b <= l) return this.Identity;
...
>>>>>>> c785d32cdb3f2869ab881fc7299aca569f8b3130
	...
	public void Solve() {
		...
		var (l, r) = readlongt2();
		ans = new List<(long, long)>();
		var Seg = new SegmentTree();
		Seg.Query(l, r, 0, 0, (long)Pow(2, Ceiling(Log2(r))));

		writeline(ans.Count);
		foreach (var an in ans) {
			writeline(an.Item1 + " " + an.Item2);
		}
<<<<<<< HEAD
