...
class Kyopuro {
	...
	public void Solve() {
		...
		long lmax = 10000 * 10000 + 100;
		double sqlmax = Sqrt(lmax);
		var prr = new List<long>();
		prr.Add(2);
		var arr = new bool[lmax];
		for (long p = 2; p <= sqlmax; p += 2) {
			arr[p] = true;
		}


		for (long p = 3; p <= sqlmax; p += 2) {
			if (arr[p]) continue;
			prr.Add(p);
			long pp = p;
			while (pp < lmax) {
		...
		while (ans <= n) {
			...
			// writeline($"ans:{ans} index:{index} prr:{prr[index]}");
			index += 1;
		}
		writeline(index);
