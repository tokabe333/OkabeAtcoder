...
bool check(const vll &arr, ll k, ll pnum) {
    ll ninzu = 0;
    rep(i, arr.size()) {
        ninzu += min(arr[i], pnum);
    }
    // printf("pnum:%lld k:%lld ninzu:%lld\n", pnum, k, ninzu);
    return pnum * k <= ninzu;
}
...
int main() {
    ...
    ll left = 0, right = 1145141919810, mid;
    ...
    if (check(arr, k, mid) == false) mid -= 1;
    cout << mid << endl;
