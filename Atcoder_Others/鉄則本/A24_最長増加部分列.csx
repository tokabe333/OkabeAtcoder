...
public class SegmentTree {
	...
	public Node Query(int l, int r, long num) {
		return this.Query(l, r, 0, 0, this.LeafNum, num);
	} // end of method
	...
	private Node Query(int l, int r, int k, int a, int b, long num) {
		...
		// 左半分と右半分で見る
		int m = (a + b) / 2;
		Node leftValue = Query(l, r, k * 2 + 1, a, m, num);
		Node rightValue = Query(l, r, k * 2 + 2, m, b, num);
