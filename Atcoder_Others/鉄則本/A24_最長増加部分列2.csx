...
public class Kyopuro {
	...
	public int LowerBound<T>(T[] a, T v, int rr = -1) {
		return LowerBound(a, v, rr, Comparer<T>.Default);
	}

	public int LowerBound<T>(T[] a, T v, int rr, Comparer<T> cmp) {
		var l = 0;
		int r = rr == -1 ? a.Length - 1 : rr;
		while (l <= r) {
			var mid = l + (r - l) / 2;
	...
	public static int UpperBound<T>(T[] a, T v, int rr = -1) {
		return UpperBound(a, v, rr, Comparer<T>.Default);
	}

	public static int UpperBound<T>(T[] a, T v, int rr, Comparer<T> cmp) {
		var l = 0;
		int r = rr == -1 ? a.Length - 1 : rr;
		while (l <= r) {
			var mid = l + (r - l) / 2;
	...
	public void Solve() {
		...
		var dp = new long[n];
		var L = makearr<long>(n, long.MaxValue);

		// printlist(L);
		int LEN = 0;
		for (int i = 0; i < n; ++i) {
			int pos = LowerBound(L, arr[i], LEN);
			dp[i] = pos + 1;
