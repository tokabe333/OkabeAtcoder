...
class Kyopuro {
	...
	/// 隣接行列についてワーシャルフロイド法で全頂点間の最短距離を作成
	/// 対角成分に0以外が入っている場合は負の閉路が存在する
	/// 隣接行列の初期値
	/// 0 (i = j)
	/// d[i][j] (exist edge(i, j))
	/// inf (otherwise, not exist edge(i, j))
	...
	public void Solve() {
		...
		/// d[i][j] (exist edge(i, j))
		/// inf (otherwise, not exist edge(i, j))
		var mat = makearr2<long>(v, v, linf);
		for (int i = 0; i < v; ++i) mat[i][i] = 0;

		for (int i = 0; i < e; ++i) {
			(int a, int b, long d) = readintt3();
			mat[a][b] = d;
		}

		var dists = WarshallFloyd(mat);
		...
		for (int i = 0; i < v; ++i) {
			for (int j = 0; j < v; ++j) {
				if (dists[i][j] == linf) write("INF");
				else write(dists[i][j]);
